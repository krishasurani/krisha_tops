...
public:
  ...
  std::string getTriangleType() const {
    ...
    } else if (std::abs(side1 - side2) < 1e-6 && std::abs(side1 - side3) < 1e-6) {
      // Use a small tolerance to account for floating-point precision errors
      return "Equilateral Triangle";
    } else if (std::abs(side1 - side2) < 1e-6 || std::abs(side2 - side3) < 1e-6 || std::abs(side1 - side3) < 1e-6) {
...
int main() {
  // Test cases for different triangle types
  Triangle triangle1(3, 3, 3);
  Triangle triangle2(4, 4, 5);
  Triangle triangle3(5, 7, 8);
  Triangle invalidTriangle(1, 1, 3); // Invalid triangle (violates triangle inequality)

  std::cout << "Triangle 1: " << triangle1.getTriangleType() << std::endl;
  std::cout << "Triangle 2: " << triangle2.getTriangleType() << std::endl;
  std::cout << "Triangle 3: " << triangle3.getTriangleType() << std::endl;
  std::cout << "Invalid Triangle: " << invalidTriangle.getTriangleType() << std::endl;
